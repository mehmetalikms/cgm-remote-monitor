...
const env = require('../../secrets'); // EKLENEN SATIR

function init (envInput, ctx) {
...
if (env.apnsKey === undefined || env.apnsKey.length == 0) {
completion("Loop notification failed: LOOP_APNS_KEY not set.");
return;
}

if (env.apnsKeyId === undefined || env.apnsKeyId.length == 0) {
completion("Loop notification failed: LOOP_APNS_KEY_ID not set.");
return;
}

if (env.apnsTeamId === undefined || env.apnsTeamId.length != 10) {
